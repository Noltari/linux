...
static int emc2101_temp_neg_read(struct emc2101_data *data, unsigned int reg, long *temp)
{
	unsigned int val;
	int ret;
...
static int emc2101_temp_pos_read(struct emc2101_data *data, unsigned int reg, long *temp)
{
	unsigned int val;
	int ret;
...
static int emc2101_temp_frac_read(struct emc2101_data *data, const unsigned int *regs, long *temp)
{
	...
	if (!ret) {
		const s8 val_hi = read_seq[0] & 0xff;
		const u8 val_lo = read_seq[1] >> TEMP_LO_SHIFT;
...
{
	const s8 val_hi = clamp_val(temp / 1000, TEMP_MIN, TEMP_MAX);
	...
	write_seq[0].def = val_hi;
	write_seq[1].def = val_lo;
					   ...
					   struct device_attribute *devattr, long lut_temp)
{
	...
	for (i = 0; i < FAN_LUT_COUNT; i++) {
		...
		ret = emc2101_temp_pos_read(data, reg, &cur_temp);
		if (ret)
			return ret;
					     ...
					     char *buf)
{
	...
	ret = emc2101_temp_pos_read(data, REG_FAN_LUT_HYST, &lut_hyst);
	if (ret)
		return ret;
					...
					char *buf)
{
	...
	ret = emc2101_temp_neg_read(data, REG_TEMP_EXT_FORCE, &temp_force);
	if (ret)
		return ret;
...
static int emc2101_fan_min_write(struct device *dev, long val)
{
	...
	if (val)
		tach_count = clamp_val(FAN_RPM_FACTOR / val, 1, FAN_MIN_READ);
	else
		tach_count = 0xffff;
...
static int emc2101_temp_ext_crit_hyst_read(struct device *dev, long *val)
{
	...
	return emc2101_temp_pos_read(data, REG_TEMP_EXT_CRIT_HYST, val);
}
...
static int emc2101_temp_ext_crit_read(struct device *dev, long *val)
{
	...
	return emc2101_temp_pos_read(data, REG_TEMP_EXT_CRIT, val);
}
...
static int emc2101_temp_int_max_read(struct device *dev, long *val)
{
	...
	return emc2101_temp_pos_read(data, REG_TEMP_INT_MAX, val);
}
...
static int emc2101_temp_int_input_read(struct device *dev, long *val)
{
	...
	return emc2101_temp_neg_read(data, REG_TEMP_INT, val);
}
